...
# Δημιουργία κλάσης TF-IDF 
class TFIDF:
    def __init__(self, documents):          # Κατασκευή μοντέλου TF-IDF
        self.documents = documents
        self.idf = self.compute_idf()

    def compute_tf(self, document):         # Υπολογισμός TF
    ...
    def compute_idf(self):                  # Υπολογισμός IDF
    ...
    def compute_tfidf(self, document):      # Υπολογισμός TF-IDF
    ...
    def rank(self, query):                  # Κατάταξη των εγγράφων
        ...
        return ranked_docs
# Δημιουργία κλάσης Vector Space Model
class VectorSpaceModel:
    def __init__(self, documents):                          # Κατασκευή μοντέλου VSM
        self.documents = documents
        self.tfidf = TFIDF(documents)

    def compute_cosine_similarity(self, vec1, vec2):        # Υπολογισμός ομοιότητας συνημιτόνου
...
# Δημιουργία κλάσης Okapi BM25 
class OkapiBM25:                                            
    def __init__(self, documents, k1=1.5, b=0.75):              # Κατασκευή μοντέλου Okapi BM25
        self.documents = documents
        self.k1 = k1
        self.b = b
    ...
    def compute_avg_doc_len(self):                              # Υπολογισμός μέσου μήκους εγγράφων
    ...
    def compute_bm25(self, document, query_tokens):             # Υπολογισμός BM25
...
# Μέθοδος για την αξιολόγηση του μοντέλου ανάκτησης πληροφορίας
...
# Λήψη Απαραίτητων Βιβλιοθηκών
nltk.download('punkt')
nltk.download('stopwords')
nltk.download('wordnet')

# Λήψη Περιεχομένου Ιστοσελίδας
url = "https://en.wikipedia.org/wiki/Enron"
page = requests.get(url)
soup = bs(page.text, 'html.parser')

# Εξαγωγή κειμένου από το κυρίως περιεχόμενο της ιστοσελίδας
...
#print("Inverted Index:", {k: v[:5] for k, v in inverted_index.items()})  # Εκτύπωση των πρώτων 5 εγγραφών για συντομία
...
# Κατασκευή μοντέλων ανάκτησης πληροφορίας
tfidf = TFIDF(documents)
vsm = VectorSpaceModel(documents)
bm25 = OkapiBM25(inverted_index)

# Τεστ ερωτήματα για την αξιολόγηση της απόδοσης του κάθε μοντέλου
test_queries = {
    "energy market": [0],
    "financial scandal": [0],
...
# Μενού για την επιλογή μοντέλου ανάκτησης πληροφορίας
while True:
    print("\nChoose an option:")
